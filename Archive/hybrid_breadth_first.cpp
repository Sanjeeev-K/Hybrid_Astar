...
bool compareInterval(maze_s i1, maze_s i2) //******************************************************************************
{ 
    return (i1.f < i2.f); 
} 
...
//   }
    ...
    open.push_back(0,state.x,state.y, 0);
    explored[state.x][state.y] = 2;
    vector<vector> delta = { {1,0}, {0,-1}, {-1,0}, {0,1} };
    ...
    while(open.size()!=0){
        ...
        for(int i = 0; i<delta.size(); i++){
            ...
            if((x_neighbor < 0 || x_neighbor2 >= grid.size()) || (y_neighbor < 0 || y_neighbor >= grid[0].size()))
                continue;
  ...
  bool finished = false;
  while(!opened.empty()) {
    sort(opened.begin(), opened.end(), compareInterval); //Sanjeev.***********************************
    ...
    for(int i = 0; i < next_state.size(); ++i) {
      ...
      double h = (x2-goal[0])*(x2-goal[0]) + (y2-goal[1])*(y2-goal[1]); //******************************
      next_state[i].f = g2 + h; //                                          ***************************
