...
vector<vector<double>> hybridastar(vector <double> START, vector <double> GOAL, vector<vector<int>> GRID){

    //Plot Grid and Start locations --PLOT STARTS HERE
...
//   }                  --PLOT ENDS HERE.
    ...
    Element Parent_of_goal(0.0,0.0,0.0);
    // cout<<Parent_of_goal.x;
    // Element q();
    ...
    while(!open_list.empty()){
        ...
        if(int(x)==GOAL[0] && int(y)==GOAL[1]){
            cout<<"Reached Goal";
            reached_goal = true;
            cout<<"Debugging here--Reached Goal"<<endl;
            cout<<theta<<" "<<theta_id(theta)<<" "<<int(x)<<" "<<GRID.size()<<" "<<int(y)<<" "<<GRID[0].size()<<endl;
            // cout<<parent.size()<<" "<<parent[0][0].size()<<" "<<parent[0].size()<<" "<<endl;
            // parent[theta_id(theta)][int(x)][int(y)].theta<<endl;
            // Parent_of_goal.x = parent[theta_id(theta)][int(x)][int(y)].x;
            // Parent_of_goal.y = parent[theta_id(theta)][int(x)][int(y)].y;
            // Parent_of_goal.theta = parent[theta_id(theta)][int(x)][int(y)].theta;
            // q = parent[theta_id(theta)][int(x)][int(y)];
            // cout<<q.x;
            cout<<"Break here";
            break;
        }
            ...
            for(auto i:neighbors){
                if(i.x>=0 && i.x<=GRID.size() && i.y>=0 && i.y<=GRID[0].size() && GRID[int(i.x)][int(i.y)]==0){
                    ...
                    if(explored[theta_neighbor_id][int(i.x)][int(i.y)]==0){
                        ...
                        cout<<"Added Valid Neighbor at x,y = ("<<i.x<<" "<<i.y<<" "<<i.theta<<" "<<Normalize(i.theta)<<" "<<theta_id(i.theta)<<" "<<theta_neighbor_id<<endl;
    ...
    for(int i = path.size(); i>=0; i--){
        double x_1 = path[i].x;
        double y_1 = path[i].y;
        double t_1 = path[i].theta;
        // plot_path(x_1,y_1);
        // plt::pause(.1);
        vector<double> element_of_result{};
        element_of_result.push_back(x_1);
        element_of_result.push_back(y_1);
        element_of_result.push_back(t_1);
        result.push_back(element_of_result);
    }

    //Coordinates of Goal
    vector<double> element_of_result{};
    element_of_result[0] = GOAL[0];
    element_of_result[1] = GOAL[1];
    element_of_result[2] = 0.0; //Final orientation set at 0.0
    result.push_back(element_of_result);
    ...
    //Function ends here. Return Vecctor of nodes.
    return result;
} 
